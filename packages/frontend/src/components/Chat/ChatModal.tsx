...
const ChatModal: React.FC<ChatModalProps> = ({
    sellerUserId, sellerName, igdbGameId, gameName, onClose, onUnreadChange, existingConvId,
}) => {
    const { user } = useAuth();
    ...
    const connectWS = useCallback(async (cid: number) => {
        try {
            const tokenRes = await api.get('/auth/ws-token');
            const token = tokenRes.data.token;
            const ws = new WebSocket(`${WS_URL}?token=${encodeURIComponent(token)}`);
            wsRef.current = ws;
            ws.onopen = () => ws.send(JSON.stringify({ type: 'join', convId: cid }));
            ws.onmessage = (e) => {
                try {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                const hist = await api.get(`/chat/conversations/${existingConvId}/messages`);
    ...
    const sendMessage = useCallback(async () => {
        ...
        if (!convIdRef.current) {
            try {
                const r = await api.post('/chat/conversations', { sellerUserId, igdbGameId, gameName });
        ...
        wsRef.current.send(JSON.stringify({ type: 'message', convId: cid, content }));
        setInput('');
    ...
    const handleEdit = async (msgId: number) => {
        ...
        try {
            ...
            setMessages(prev => prev.map(m =>
                m.id === msgId ? { ...m, content: editText.trim(), is_edited: true } : m
            ));
            setEditingId(null);
            setEditText('');
        } catch { }
    ...
    const handleDelete = async (msgId: number) => {
        try {
            ...
            setMessages(prev => prev.map(m =>
                m.id === msgId ? { ...m, content: 'Сообщение удалено', is_deleted: true } : m
            ));
        } catch { }
    };
                                    ...
                                    {editingId === msg.id ? (
                                        <div className="chat-edit-row">
                                            <input
                                                ...
                                                autoFocus
                                            />
                                            <button className="chat-edit-save" onClick={() => handleEdit(msg.id)}>✓</button>
                                            <button className="chat-edit-cancel" onClick={() => setEditingId(null)}>✕</button>
                                        </div>
                                    ) : (
                                        <div className={`chat-bubble ${msg.is_deleted ? 'chat-bubble--deleted' : ''}`}>
                                            {msg.content}
                                    ...
                                    <div className="chat-msg-meta">
                                        ...
                                        {isMine && !msg.is_deleted && editingId !== msg.id && (
                                            <div className="chat-msg-actions">
                                                <button className="chat-action-btn" onClick={() => { setEditingId(msg.id); setEditText(msg.content); }}>Изменить</button>
                                                <button className="chat-action-btn chat-action-btn--del" onClick={() => handleDelete(msg.id)}>Удалить</button>
                                            </div>
                                        )}
                                    </div>
                                </div>
