...
const UserGamesPage: React.FC = () => {
  ...
  const carousel = useInfiniteCarousel(
    Math.max(similarGames.length, 1),
    VISIBLE
  );
  const extendedSimilar =
    similarGames.length >= VISIBLE
      ? [
          ...similarGames.slice(-VISIBLE),
          ...similarGames,
          ...similarGames.slice(0, VISIBLE),
        ]
      : similarGames;
          ...
          <div className="ugp-carousel">
            ...
            <div className="ugp-carousel-window">
              <div
                ...
                className="ugp-carousel-track"
                style={{
                  transform: `translateX(-${carousel.index * CARD_W}px)`,
                  transition: carousel.withTransition
                    ? 'transform 0.4s ease'
                    : 'none',
                }}
              >
                {extendedSimilar.map((g, idx) => {
                  ...
                  return (
                    <div
                      key={`${g.id}-${idx}`}
                      className="ugp-carousel-item"
                      onClick={() => navigate(`/game/${g.slug || g.id}`)}
                    >
                      {cover ? (
                        <img src={cover} alt={g.name} />
                      ) : (
                        <div className="ugp-carousel-no-cover">
                          Нет изображения
                        </div>
                      )}
                      <div className="ugp-carousel-info">
                        ...
                        {g.rating !== undefined && (
                          <span className="ugp-carousel-rating">
                            ...
                            {g.genres
                              ...
                              .map((genre: any) => (
                                <span
                                  ...
                                  className="ugp-carousel-genre-tag"
                                >
                                  {genre.name}
                                </span>
                              ))}
