...
const UserPage: React.FC = () => {
  ...
  const [orderActionLoading, setOrderActionLoading] = useState<number | null>(
    null
  );
  ...
  const [editPriceListingId, setEditPriceListingId] = useState<number | null>(
    null
  );
  ...
  const [orderGameCovers, setOrderGameCovers] = useState<
    Record<number, string>
  >({});
  const [orderGameSlugs, setOrderGameSlugs] = useState<
    Record<number, string | number>
  >({});

  const [sellerOrderGameNames, setSellerOrderGameNames] = useState<
    Record<number, string>
  >({});
  const [sellerOrderGameCovers, setSellerOrderGameCovers] = useState<
    Record<number, string>
  >({});
  const [sellerOrderGameSlugs, setSellerOrderGameSlugs] = useState<
    Record<number, string | number>
  >({});
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (user.role === 'seller') {
        api
          .get(`/reviews/seller/${user.id}`)
          .then(async (r) => {
            const reviews = r.data.reviews || [];
            setSellerReviews(reviews);
            if (reviews.length > 0) {
              const ids = [
                ...new Set(reviews.map((rv: any) => rv.igdb_game_id)),
              ];
              try {
                const res = await api.post('/games/bulk', { ids });
                const names: Record<number, string> = {};
      ...
      const { getUserComments, getGamesBulk } =
        await import('../services/api').catch(() => ({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (newPassword && !currentPassword)
      return setError('Требуется текущий пароль для смены пароля');
  ...
  const handleSendKey = async (orderId: number) => {
    ...
    setOrderActionLoading(orderId);
    try {
      await api.post(`/orders/${orderId}/send-key`, {
        gameKey: gameKeyInput.trim(),
      });
      setSellerOrders((prev) =>
        prev.map((o) =>
          o.id === orderId
            ? { ...o, status: 'key_sent', game_key: gameKeyInput.trim() }
            : o
        )
      );
  ...
  const handleSubmitReview = async (order: any) => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((o) => (o.id === order.id ? { ...o, has_review: 1 } : o))
      );
  ...
  const handleSaveDescription = async () => {
    setDescLoading(true);
    try {
      await api.patch('/sellers/description', {
      ...
      setProfile((prev) =>
        prev?.seller
          ? {
              ...prev,
              seller: { ...prev.seller, description: sellerDescription.trim() },
            }
          : prev
      );
        ...
        <div className="up-container">
          <div className="up-card">
            <p className="up-muted">
              Войдите, чтобы посмотреть ваш профиль:{' '}
              <Link to="/auth" className="up-link">
                вход
              </Link>
              .
            </p>
      ...
      <div className="up-container">
        <div className="up-card">
          <div className="up-profile-header">
            ...
            <div className="up-profile-meta">
              ...
              {joinedDate && (
                <p className="up-joined">Участник с {joinedDate}</p>
              )}
            </div>
            <button
        ...
        {user.role === 'seller' && profile?.seller && (
          <div className="up-card">
            <div className="up-stats-row">
              ...
              <div className="up-stat">
                <span className="up-stat-value">
                  {profile.seller.listings?.length ?? 0}
                </span>
              ...
              {editingDescription ? (
                <div className="up-field">
                  ...
                  <div className="up-form-actions" style={{ marginTop: 8 }}>
                    ...
                    >
                      {descLoading ? '...' : 'Сохранить'}
                    </button>
                    <button
                      className="up-cancel-order-btn"
                ...
                >
                  <p className="up-description" style={{ flex: 1, margin: 0 }}>
                    {profile.seller.description || (
                      <span className="up-muted">Пока нет описания.</span>
                    )}
                  </p>
                  <button
                    className="up-edit-price-btn"
                    onClick={() => {
                      setSellerDescription(profile.seller?.description || '');
        ...
        {editMode && (
          <div className="up-card">
            ...
            <form className="up-form" onSubmit={handleSave}>
              ...
              <p className="up-hint">
                Оставьте поля пароля пустыми, чтобы сохранить текущий пароль
              </p>
              <div className="up-field">
            ...
            {showDelete && (
              <div className="up-delete-box">
                ...
                <div className="up-form-actions">
                  ...
                  <button className="up-danger-btn" onClick={handleDelete}>
        ...
        {user.role !== 'seller' && (
          <div className="up-card">
            <h2 className="up-section-title">
              Заказы
              {orders.length > 0 && (
                <span className="up-count">{orders.length}</span>
              )}
            </h2>
            {ordersLoading ? (
              ...
              <p className="up-muted">
                Пока нет заказов.{' '}
                <Link to="/explore" className="up-link">
                  Просмотреть игры
                </Link>
              </p>
            ) : (
              <div className="up-orders-list">
                {orders.map((order) => {
                  const gameName =
                    orderGameNames[order.igdb_game_id] ||
                    `Игра №${order.igdb_game_id}`;
                  const gameCover = orderGameCovers[order.igdb_game_id];
                  const gameSlug =
                    orderGameSlugs[order.igdb_game_id] || order.igdb_game_id;
                  ...
                  return (
                    <div key={order.id} className="up-order-card">
                      <Link
                        to={`/game/${gameSlug}`}
                        className="up-order-cover-link"
                      >
                        {gameCover ? (
                          <img
                            className="up-order-cover"
                            src={gameCover}
                            alt={gameName}
                          />
                        ) : (
                          <div className="up-order-cover up-order-cover--empty">
                            🎮
                          </div>
                        )}
                      </Link>
                      ...
                      <div className="up-order-body">
                        <div className="up-order-top">
                          <Link
                            to={`/game/${gameSlug}`}
                            className="up-order-game-name"
                          >
                            {gameName}
                          </Link>
                          <span className="up-order-price">
                        ...
                        <div className="up-order-meta">
                          ...
                          <Link
                            to={`/profile/${order.seller_user_id}`}
                            className="up-order-seller-link"
                          >
                            {order.seller_name}
                          </Link>
                        </div>
                        ...
                        {order.status === 'completed' &&
                          (order.game_key || !order.has_review) && (
                            <div className="up-order-key-row">
                              {order.game_key && (
                                <>
                                  <span className="up-order-key-label">
                                    🔑 Steam-ключ:
                                  </span>
                        ...
                        {order.status === 'completed' &&
                          ...
                          reviewOrderId === order.id && (
                            <div className="up-review-form">
                              <div className="up-stars-row">
                                {[1, 2, 3, 4, 5].map((s) => (
                                  <button
                                    key={s}
                                    type="button"
                                    className={`up-star ${reviewRating >= s ? 'up-star--active' : ''}`}
                              ...
                              <textarea
                                className="up-review-input"
                                placeholder="Напишите ваш отзыв (по желанию)..."
                                value={reviewText}
                      ...
                      <div className="up-order-status-col">
                        <span
                          className="up-order-status-badge"
                          style={{
                            color: st.color,
                ...
                return (
                  <div key={comment.id} className="up-comment-item">
                    {coverUrl ? (
                      ...
                      <div
                        className="up-comment-cover up-comment-cover--empty"
                        onClick={() => navigate(`/game/${gameSlug}`)}
                      >
                        🎮
                      </div>
                    ...
                    <div className="up-comment-body">
                      <button
                        className="up-comment-game-link"
                        onClick={() => navigate(`/game/${gameSlug}`)}
                      >
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={async () => {
                                ...
                                setMyComments((prev: any[]) =>
                                  prev.filter((c: any) => c.id !== comment.id)
                                );
                              }}
                            >
                              Удалить
                    ...
                    <span className="up-comment-date">
        ...
        {user.role === 'seller' && (
          <div className="up-card">
            <h2 className="up-section-title">
              Заказы
              {sellerOrders.filter((o) => o.status === 'pending_key').length >
                0 && (
                <span className="up-count up-count-alert">
                  {
                    sellerOrders.filter((o) => o.status === 'pending_key')
                      .length
                  }{' '}
                  новых
                </span>
            ...
            {sellerOrdersLoading ? (
              ...
              <div className="up-orders-list">
                {sellerOrders.map((order) => {
                  const gameName =
                    sellerOrderGameNames[order.igdb_game_id] ||
                    `Игра №${order.igdb_game_id}`;
                  const gameCover = sellerOrderGameCovers[order.igdb_game_id];
                  const gameSlug =
                    sellerOrderGameSlugs[order.igdb_game_id] ||
                    order.igdb_game_id;
                  const statusMap: Record<
                    string,
                    { label: string; color: string }
                    ...
                    <div key={order.id} className="up-order-card">
                      ...
                      <div className="up-order-body">
                        ...
                        <div className="up-order-meta">
                          <span className="up-order-id">Заказ №{order.id}</span>
                          <span className="up-order-dot">·</span>
                          <span className="up-order-seller-link">
                            {order.buyer_name}
                          </span>
                        </div>
                        {order.status === 'pending_key' && (
                          <div className="up-order-key-row">
                            {sendKeyOrderId === order.id ? (
                              <>
                                <input
                                  className="up-key-input"
                                  placeholder="Введите ключ игры (например, XXXX-XXXX-XXXX)"
                                  value={gameKeyInput}
                                  onChange={(e) =>
                                ...
                                <button
                                  ...
                                  onClick={() => handleSendKey(order.id)}
                                >
                                  {orderActionLoading === order.id
                                    ? '...'
                                    : 'Отправить ключ'}
                              ...
                              <button
                                className="up-confirm-btn"
                                onClick={() => setSendKeyOrderId(order.id)}
                              >
                                Отправить ключ игры
                              </button>
                            )}
        ...
        {user.role === 'seller' && profile?.seller && (
          <div className="up-card">
            <h2 className="up-section-title">
              Мои объявления
              {profile.seller.listings && (
                <span className="up-count">
                  {profile.seller.listings.length}
                </span>
              )}
            </h2>
            {!profile.seller.listings ||
            profile.seller.listings.length === 0 ? (
              ...
              (() => {
                ...
                const slice = profile.seller.listings.slice(
                  page * PAGE_SIZE,
                  (page + 1) * PAGE_SIZE
                );
                return (
                    ...
                    <div className="up-listings">
                      {slice.map((l: any) => (
                        <div key={l.id} className="up-listing-card">
                          <Link
                            to={`/game/${l.igdb_game_id}`}
                            className="up-listing-cover-link"
                          >
                            {listingGameCovers[l.igdb_game_id] ? (
                              <img
                                className="up-listing-cover"
                                src={listingGameCovers[l.igdb_game_id]}
                                alt={listingGameNames[l.igdb_game_id] || ''}
                              />
                            ) : (
                              <div className="up-listing-cover up-listing-cover--empty">
                                🎮
                              </div>
                            )}
                          </Link>
                          <div className="up-listing-body">
                            <div className="up-listing-body-left">
                              <Link
                                to={`/game/${l.igdb_game_id}`}
                                className="up-listing-name"
                              >
                                {listingGameNames[l.igdb_game_id] ||
                                  `Игра №${l.igdb_game_id}`}
                              </Link>
                              <span className="up-listing-id">
                                #{l.igdb_game_id}
                              </span>
                            </div>
                    ...
                    <span className="up-review-stars">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <span
                          key={s}
                          style={{
                            color: s <= r.rating ? '#fbbf24' : '#30363d',
                          }}
                        >
                          ★
                        </span>
                    ...
                    <div className="up-review-reply-form">
                      ...
                      <div className="up-form-actions" style={{ marginTop: 6 }}>
                        <button
                          ...
                          onClick={() => handleReplyToReview(r.id)}
                        >
                          {replyLoading ? '...' : 'Сохранить'}
                        </button>
                        <button
                          className="up-cancel-order-btn"
