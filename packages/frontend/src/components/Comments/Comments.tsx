...
const CommentItem: React.FC<CommentItemProps> = ({ comment, gameId, user, isAuthenticated, onReload, depth = 0 }) => {
    ...
    return (
        <div className={`comments__item ${depth > 0 ? 'comments__item--reply' : ''}`}>
            <div className="comments__header">
                <div className="comments__author">
                    <Link to={`/profile/${comment.user_id}`} className="comments__author-link">
                    ...
                    {isAuthenticated && user && user.id === comment.user_id && (
                        <>
                            {editingId === comment.id ? (
                                <>
                                    <button type="button" onClick={() => handleUpdate(comment.id)} className="comments__action-btn comments__action-btn--save">Сохранить</button>
                                    <button type="button" onClick={() => setEditingId(null)} className="comments__action-btn comments__action-btn--cancel">Отмена</button>
                                </>
                            ) : (
                                <>
                                    <button onClick={() => { setEditingId(comment.id); setEditText(comment.comment_text); }} className="comments__action-btn comments__action-btn--edit">Изменить</button>
                                    <button onClick={() => handleDelete(comment.id)} className="comments__action-btn comments__action-btn--delete">Удалить</button>
                                </>
                            )}
                        </>
                    )}
            ...
            {editingId === comment.id ? (
                <textarea className="comments__edit-input" value={editText} onChange={e => setEditText(e.target.value)} rows={3} />
            ) : (
                <div>
            ...
            {replyOpen && (
                <div className="comments__reply-form">
                    ...
                    <div className="comments__reply-actions">
                        <button className="comments__submit-btn comments__submit-btn--sm" disabled={!replyText.trim() || replyLoading} onClick={handleReply}>
                            {replyLoading ? '...' : 'Отправить ответ'}
                        </button>
                        <button className="comments__action-btn comments__action-btn--cancel" onClick={() => setReplyOpen(false)}>Отмена</button>
                    </div>
                </div>
            )}
    ...
    return (
        <div className="comments">
            ...
            {loading ? (
                ...
                <div className="comments__empty">Пока нет комментариев. Будьте первым!</div>
            ) : (
