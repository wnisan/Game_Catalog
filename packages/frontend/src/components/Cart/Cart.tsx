...
import { getCart, removeFromCart, type CartItem, api } from '../../services/api';
import './Cart.css';
...
const Cart = ({ onClose, onCartChange, onBalanceChange }: CartProps) => {
    ...
    const handleRemove = async (listingId: number) => {
        try {
            ...
            window.dispatchEvent(new CustomEvent('cartItemRemoved', { detail: { listingId } }));
        } catch {  }
    };
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            setCheckoutError(err.response?.data?.error || 'Не удалось оформить заказ');
        } finally {
        ...
        <div className="cart-overlay" onClick={onClose}>
            <div className="cart-panel" onClick={e => e.stopPropagation()}>
                <div className="cart-panel__header">
                    ...
                    <button className="cart-panel__close" onClick={onClose} aria-label="Закрыть корзину">✕</button>
                </div>
                    ...
                    {loading ? (
                        ...
                        <div className="cart-checkout-success">
                            ...
                            <p className="cart-checkout-sub">
                                Создано {orderCount} {orderWord(orderCount)}. Продавец скоро пришлёт вам ключ игры.
                                Проверьте ваши заказы на странице профиля.
                            </p>
                            <button className="cart-panel__checkout-btn" onClick={onClose}>Закрыть</button>
                        </div>
                    ) : items.length === 0 ? (
                        ...
                        items.map(item => (
                            <div key={item.id} className="cart-item">
                                <div className="cart-item__cover-wrap" onClick={() => { onClose(); navigate(`/game/${gameSlugs[item.igdb_game_id] || item.igdb_game_id}`); }}>
                                    {gameCovers[item.igdb_game_id] ? (
                                        <img className="cart-item__cover" src={gameCovers[item.igdb_game_id]} alt={gameNames[item.igdb_game_id] || ''} />
                                    ) : (
                                        <div className="cart-item__cover-placeholder">🎮</div>
                                    )}
                                </div>
                                <div className="cart-item__info">
                                    <p className="cart-item__name" onClick={() => { onClose(); navigate(`/game/${gameSlugs[item.igdb_game_id] || item.igdb_game_id}`); }}>
                                        {gameNames[item.igdb_game_id] || `Игра №${item.igdb_game_id}`}
                                    </p>
                                    <p className="cart-item__seller" onClick={() => { onClose(); navigate(`/profile/${item.seller_profile_id}`); }}>
                                        {item.seller_name}{item.is_verified && ' ✓'}
                                    </p>
                                    <p className="cart-item__price">${Number(item.price).toFixed(2)}</p>
                                </div>
                                <button className="cart-item__remove" onClick={() => handleRemove(item.listing_id)} aria-label="Удалить из корзины">✕</button>
                            </div>
                        ))
                    )}
                </div>
                ...
                {!checkoutDone && items.length > 0 && (
                    <div className="cart-panel__footer">
                        <div className="cart-panel__total">
                            <span>Итого</span>
                            <span className="cart-panel__total-amount">${total.toFixed(2)}</span>
                        </div>
                        ...
                        >
                            {checkoutLoading ? 'Обработка...' : `Оплатить $${total.toFixed(2)}`}
                        </button>
                    </div>
                )}
            </div>
