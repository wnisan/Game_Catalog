...
const CheckoutForm: React.FC<{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (submitErr) {
      setError(submitErr.message || 'Ошибка');
      setLoading(false);
      return;
    }
...
const TopUpModal: React.FC<TopUpModalProps> = ({ onClose, onSuccess }) => {
  ...
  const handleProceed = async () => {
    if (!finalAmount || finalAmount < 1)
      return setError('Минимальная сумма — $1');
    if (finalAmount > 10000) return setError('Максимальная сумма — $10,000');
    setLoading(true);
    setError('');
    try {
      const res = await api.post('/payments/create-topup-intent', {
        amount: finalAmount,
      });
      ...
      setError(
        err.response?.data?.error || 'Не удалось инициализировать платеж'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSuccess = async () => {
    setStep('done');
    try {
      const res = await api.post('/payments/confirm-topup', {
        paymentIntentId,
      });
    ...
    >
      <div className="tum-modal">
        <div className="tum-header">
          ...
          <button className="tum-close" onClick={onClose}>
            ✕
          </button>
        </div>
        ...
        {step === 'select' && (
          <div className="tum-body">
            ...
            >
              {loading
                ? 'Загрузка...'
                : `Продолжить — $${(finalAmount || 0).toFixed(2)}`}
            </button>
          </div>
        )}
        ...
        {step === 'pay' && clientSecret && (
          <div className="tum-body">
            <p className="tum-amount-display">
              Добавляем <strong>${finalAmount.toFixed(2)}</strong> к вашему
              балансу
            </p>
            <Elements
        ...
        {step === 'done' && (
          <div className="tum-body tum-done">
            ...
            <p className="tum-done-sub">
              ${finalAmount.toFixed(2)} добавлено к вашему балансу.
            </p>
            <button className="tum-pay" onClick={onClose}>
              Закрыть
            </button>
          </div>
        )}
