...
import {
  FilterStatsResponseSchema,
  GamesResponseSchema,
} from '@game-catalog/shared';
import type {
  Comment,
  Filter,
  FilterStatsResponse,
...
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('unhandledrejection', (event) => {
    ...
    if (
      (errorMessage.includes('/auth/me') || errorStack.includes('/auth/me')) &&
      (errorMessage.includes('401') || errorMessage.includes('Unauthorized'))
    ) {
      event.preventDefault();
      return false;
...
export const getGames = async (
  ...
  try {
    const params: any = {
      ...
      gameIds: filters.sellerGameIds?.length
        ? filters.sellerGameIds.join(',')
        : undefined,
    };
...
export const getFilterStats = async (
  ...
  try {
    ...
    if (error.response?.status === 429) {
      throw new Error(
        'Too many requests. Filter statistics are temporarily unavailable.'
      );
    }
    throw error;
  }
...
export const getListingForGame = async (
  gameId: number
