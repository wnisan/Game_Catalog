...
const SellerBlock = ({
  ...
  const isOwnListing =
    listing && user?.role === 'seller' && listing.seller_user_id === user.id;
  ...
  return (
    <div className="seller-block">
      ...
      {!listing ? (
        <p className="seller-block__no-listing">
          Пока нет продавцов для этой игры.
        </p>
      ) : (
        <>
          <div className="seller-block__main-row">
            ...
            <div className="seller-block__left">
              {canEditPrice && editingPrice ? (
                <div className="seller-block__price-edit">
                  ...
                  />
                  <button
                    className="seller-block__save-btn"
                  ...
                  <button
                    className="seller-block__cancel-btn"
                ...
                <div className="seller-block__price-display">
                  <span className="seller-block__price">
                    ${Number(listing.price).toFixed(2)}
                  </span>
                  {canEditPrice && (
                    <button
                      className="seller-block__edit-price-btn"
                      onClick={() => {
                        setPriceInput(Number(listing.price).toFixed(2));
            ...
            <div className="seller-block__seller-card">
              <div className="seller-block__seller-row">
                <Link
                  to={`/profile/${listing.seller_user_id}`}
                  className="seller-block__seller-link"
                >
                  {listing.seller_name}
                </Link>
                {listing.is_verified && (
                  <span className="seller-block__verified">✓</span>
                )}
                {listing.seller_rating > 0 && (
                  <span className="seller-block__rating">
          ...
          {showAuthPrompt && (
            <AuthPromptModal
              ...
              message="Войдите, чтобы покупать игры и общаться с продавцами."
            />
          )}
