...
import {
    adminGetUsers, adminBanUser, adminSetRole,
    adminGetHiddenGames, adminUnhideGame, adminHideGame,
    updateListingPrice,
    type AdminUser,
} from '../services/api';
...
const AdminPage: React.FC = () => {
    ...
    const loadUsers = async () => {
        ...
        catch { setActionMsg('Не удалось загрузить пользователей'); }
        finally { setUsersLoading(false); }
    };

    const loadHidden = async () => {
        setHiddenLoading(true);
        try { setHiddenGames(await adminGetHiddenGames()); }
        catch { setActionMsg('Не удалось загрузить скрытые игры'); }
    ...
    const loadListings = async () => {
        ...
        } catch { setActionMsg('Не удалось загрузить объявления'); }
    ...
    const handleUpdateListingPrice = async (listingId: number) => {
        ...
        try {
            ...
            setListings(prev => prev.map(l => l.id === listingId ? { ...l, price } : l));
            setEditPriceId(null);
            flash('Цена обновлена');
        } catch { flash('Не удалось обновить цену'); }
    ...
    const handleBan = async (u: AdminUser) => {
        ...
        } catch { flash('Действие не удалось'); }
    };
    ...
    const handleUnhide = async (igdbGameId: number) => {
        try {
            ...
            setHiddenGames(prev => prev.filter(g => g.igdb_game_id !== igdbGameId));
        ...
        <div className="ap-page">
            <div className="ap-container">
                ...
                <div className="ap-tabs">
                    <button className={`ap-tab ${tab === 'users' ? 'ap-tab-active' : ''}`} onClick={() => setTab('users')}>
                        Пользователи {users.length > 0 && <span className="ap-tab-count">{users.length}</span>}
                    </button>
                    <button className={`ap-tab ${tab === 'listings' ? 'ap-tab-active' : ''}`} onClick={() => setTab('listings')}>
                        Объявления
                    </button>
                    <button className={`ap-tab ${tab === 'hidden' ? 'ap-tab-active' : ''}`} onClick={() => setTab('hidden')}>
                        Скрытые игры {hiddenGames.length > 0 && <span className="ap-tab-count">{hiddenGames.length}</span>}
                    </button>
                </div>
                                    ...
                                    <tbody>
                                        {filtered.map(u => (
                                            <tr key={u.id} className={u.is_banned ? 'ap-row-banned' : ''}>
                                                ...
                                                <td>
                                                    <span className={`ap-status ${u.is_banned ? 'ap-status-banned' : 'ap-status-active'}`}>
                    ...
                    <div className="ap-card">
                        <div className="ap-search-row">
                            <input
                                ...
                                value={listingSearch}
                                onChange={e => { setListingSearch(e.target.value); setListingsPage(0); }}
                            />
                        ...
                        {listingsLoading ? (
                            ...
                            const totalPages = Math.ceil(filtered.length / LISTINGS_PAGE_SIZE);
                            const page = Math.min(listingsPage, Math.max(0, totalPages - 1));
                            const slice = filtered.slice(page * LISTINGS_PAGE_SIZE, (page + 1) * LISTINGS_PAGE_SIZE);
                            return (
                                <>
                                    <div className="ap-table-wrap">
                                        <table className="ap-table">
                                                            ...
                                                            {editPriceId === l.id ? (
                                                                <div className="ap-hide-row">
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        onClick={() => handleUpdateListingPrice(l.id)}
                                                                    >
                                                        ...
                                                        <td>
                                                            {editPriceId !== l.id && (
                                                                <button
                                                                    ...
                                                                    onClick={() => { setEditPriceId(l.id); setEditPriceVal(parseFloat(l.price).toFixed(2)); }}
                                                                >
                                                                    Изменить цену
                                                                </button>
                                                            )}
                                    ...
                                    {totalPages > 1 && (
                                        <div className="ap-pagination">
                                            <button className="ap-page-btn" onClick={() => setListingsPage(0)} disabled={page === 0}>«</button>
                                            <button className="ap-page-btn" onClick={() => setListingsPage(p => Math.max(0, p - 1))} disabled={page === 0}>‹</button>
                                            <span className="ap-page-info">{page + 1} / {totalPages} ({filtered.length} поз.)</span>
                                            <button className="ap-page-btn" onClick={() => setListingsPage(p => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1}>›</button>
                                            <button className="ap-page-btn" onClick={() => setListingsPage(totalPages - 1)} disabled={page === totalPages - 1}>»</button>
                                        </div>
                                    )}
                                </>
                            );
