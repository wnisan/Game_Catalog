  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        ...
        ws = new WebSocket(
          `${WS_URL}?token=${encodeURIComponent(r.data.token)}`
        );
        wsRef.current = ws;
        ws.onmessage = (e) => {
          try {
      ...
      <div className="app-bar__container">
        <div className="app-bar__left">
          ...
          <nav className="app-bar__nav" aria-label="Главная навигация">
            <Link
              to="/explore"
              className={`app-bar__nav-link ${isExplorePage ? 'app-bar__nav-link--active' : ''}`}
            >
              Обзор
            </Link>
            {isExplorePage && (
              ...
              >
                ...
                {activeFiltersCount > 0 && (
                  <span className="app-bar__filter-badge">
                    {activeFiltersCount}
                  </span>
                )}
              </button>
            ...
            {isAuthenticated && (
              <Link
                to="/favourites"
                className={`app-bar__nav-link ${location.pathname === '/favourites' ? 'app-bar__nav-link--active' : ''}`}
              >
                Избранное
              </Link>
            )}
        ...
        <form className="app-bar__search-form" onSubmit={handleSearch}>
          <div className="app-bar__search-wrap">
            <svg
              className="app-bar__search-icon"
              width="16"
              height="16"
              viewBox="0 0 24 24"
              fill="none"
        ...
        <div className="app-bar__right">
          {isAuthenticated ? (
            <>
              <button
                className="app-bar__nav-link app-bar__nav-btn"
              ...
              <Link
                to="/user"
                className={`app-bar__nav-link app-bar__user-link ${location.pathname === '/user' ? 'app-bar__nav-link--active' : ''}`}
              >
              ...
              {user?.role === 'admin' && (
                <Link
                  to="/admin"
                  className={`app-bar__nav-link app-bar__admin-link ${location.pathname === '/admin' ? 'app-bar__nav-link--active' : ''}`}
                >
                  Админка
                </Link>
              )}
              ...
              <button
                className="app-bar__nav-link app-bar__nav-btn app-bar__logout-link"
